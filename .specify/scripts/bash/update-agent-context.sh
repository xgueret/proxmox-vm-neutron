...
# Variables populated by get_feature_paths (declare only those used in this script)
REPO_ROOT="" CURRENT_BRANCH="" HAS_GIT="" IMPL_PLAN=""
...
update_existing_agent_file() {
  ...
  while IFS= read -r line || [[ -n $line ]]; do
    ...
    if [[ $line =~ (\*\*Last\ updated\*\*:.*)[0-9]{4}-[0-9]{2}-[0-9]{2}(.*) ]]; then
      echo "${BASH_REMATCH[1]}${current_date}${BASH_REMATCH[2]}" >>"$temp_file"
    else
      echo "$line" >>"$temp_file"
    fi
  done <"$target_file"
